...
class Listener(tweepy.Stream):
    ...
    def __init__(self, data):
        super(Listener, self).__init__(access_token=access_token,
                                       access_token_secret=access_token_secret, consumer_key=api_key, consumer_secret=api_secret)
    ...
    def on_error(self, status):
        ...
        self.message_queue.put(
            f'Received error message form Twitter API: {status}')
        return False
    ...
    def run(self):
        ...
        while not self.stoprequest.isSet():
            if len(self.keywords) > 0:
                ...
                stream = tweepy.Stream(access_token=access_token,
                                       access_token_secret=access_token_secret, consumer_key=api_key, consumer_secret=api_secret)
                sample_stream = tweepy.Stream(
                    access_token=access_token, access_token_secret=access_token_secret, consumer_key=api_key, consumer_secret=api_secret)
                ...
                sample_stream.sample(stall_warnings=True, **self.filter_params)
